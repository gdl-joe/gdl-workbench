...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    const aiCtxScripts  = document.getElementById('aiCtxScripts');
    const aiCtxParams   = document.getElementById('aiCtxParams');

    // Gespeichertes Modell wiederherstellen (geteilt mit Script Editor)
    ...
    const AI_HISTORY_KEY = 'codeSchmiedeAiHistory';

    // Verlauf aus localStorage laden
    let aiHistory = [];
    try { aiHistory = JSON.parse(localStorage.getItem(AI_HISTORY_KEY) || '[]'); } catch { aiHistory = []; }
    ...
    function aiAddMessage(role, text) {
        ...
        // "In Editor einfügen"-Button bei Assistenten-Antworten
        if (role === 'assistant' && text.includes('```')) {
            const btn = document.createElement('button');
            btn.className = 'ai-insert-btn';
            btn.innerHTML = '<i class="fas fa-arrow-right"></i> Code in Vorschau übernehmen';
            btn.addEventListener('click', () => {
                ...
                if (match) {
                    ...
                    if (pre) pre.innerHTML = highlightGDL(generatedCode);
                    setStatus('KI-Code in Vorschau übernommen – jetzt „In Editor einfügen" klicken');
                }
            });
            div.appendChild(btn);
        }
    ...
    async function aiSend() {
        ...
        const thinking = aiAddMessage('thinking', '⏳ KI denkt nach …');

        // Kontext zusammenstellen
        const context = {};
        if (aiCtxScripts && aiCtxScripts.checked) {
            // Skripte aus dem Script-Editor lesen (falls verfügbar über localStorage)
            const stored = localStorage.getItem('gdl_scripts');
            if (stored) {
                try { context.scripts = JSON.parse(stored); } catch(e) {}
            }
        }
        if (aiCtxParams && aiCtxParams.checked) {
            const stored = localStorage.getItem('gdl_parameters');
            if (stored) {
                try { context.parameters = JSON.parse(stored); } catch(e) {}
            }
        }
        ...
        try {
            ...
            if (data.success && data.data?.reply) {
                ...
                const errMsg = data.message || 'Unbekannter Fehler';
                aiAddMessage('error', '❌ ' + errMsg);
                aiHistory.pop(); // fehlgeschlagene user-Nachricht entfernen
            }
        } catch (e) {
            thinking.remove();
            aiAddMessage('error', '❌ Netzwerkfehler: ' + e.message);
            aiHistory.pop();
        }
    ...
    aiClearBtn.addEventListener('click', () => {
        ...
        aiAddMessage('assistant', 'Verlauf gelöscht. Wie kann ich Dir helfen?');
    });

    // Gespeicherten Verlauf beim Laden wiederherstellen
    if (aiHistory.length > 0) {
        aiHistory.forEach(msg => aiAddMessage(msg.role, msg.content));
    }
});
